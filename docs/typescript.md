...
Due to a known issue, the latest `@noir-lang/barretenberg` is incompatible with `noir_wasm` and
`aztec_backend`. Please make sure you install `barretenberg` via the provided github link.
